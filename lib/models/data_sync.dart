...
class HttpHelper
{
  ...
  static String? main_pass_str = null;
  ...
  static bool main_pass = false; //encrypt owner_key and custom_api in cfg file
  ...
  static Future<bool> initOwnerKey() async
  {
    try {
      ...
      {
        if (main_pass) { owner_key = decrypt_content(owner_key, main_pass_str!); }
        ...
        String _owner_key = main_pass ? encrypt_content(owner_key, main_pass_str!) : owner_key;
        return await update_config_value("owner_key", _owner_key);
      }
    } catch (e) {
      return false;
    }
  ...
  {
    try {
      String _owner_key = main_pass ? encrypt_content(key, main_pass_str!) : key;
      if (await update_config_value("owner_key", _owner_key)) {
        owner_key = key;
        owner_key_hash = hash(owner_key, rounds: 5000, salt: owner_key.split('').reversed.join().substring(0, 16));
        return true;
      }
  ...
  {
    try {
      if (main_pass) { _custom_api_url = encrypt_content(_custom_api_url, main_pass_str!); }
  ...
  static Future<bool> get_custom_api_cfg() async
  {
    try {
      ...
      if (main_pass) { custom_api_url = decrypt_content(custom_api_url, main_pass_str!); }
  ...
  static Future<bool> test_main_pass() async
  {
    try
    {
      String test_owner_key = await get_config_value("owner_key");
      if (test_owner_key.isNotEmpty) { test_owner_key = decrypt_content(test_owner_key, main_pass_str!); }
      return true;
    }
    catch (e) { return false; }
  }
